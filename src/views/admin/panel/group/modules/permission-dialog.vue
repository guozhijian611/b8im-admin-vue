...
<script setup lang="ts">
  ...
  import menuApi from '@/api/admin/panel/menu'
  ...
  const initPage = async () => {
    // 菜单列表
    menuList.value = await menuApi.list({ tree: true })
    // 角色数据
    const data = await api.getMenuByGroup({ id: props.data?.id })
