      ...
      <div class="flex-shrink-0 h-full max-md:w-full max-md:h-auto max-md:mb-5">
        <ElCard class="left-card art-card-xs flex flex-col h-full mt-0" shadow="never">
          ...
          <ElSpace wrap>
            ...
            <SaButton
              v-permission="'saimulti:config:save'"
              type="primary"
              @click="showDialog('add')"
            />
            <SaButton
              v-permission="'saimulti:config:update'"
              type="secondary"
              @click="updateConfigDialog"
            />
            <SaButton
              v-permission="'saimulti:config:destroy'"
              type="error"
              @click="deleteConfigData"
            />
          </ElSpace>
          <ArtTable
            rowKey="id"
            ...
            :paginationOptions="{ layout: 'total, prev, pager, next' }"
        ...
        <ElCard class="art-card-xs flex flex-col h-full mt-0" shadow="never">
          ...
          <div class="max-h-[calc(100vh-250px)] overflow-y-auto">
            <ElForm ref="formRef" :model="formData" label-width="140px">
              ...
              <ElFormItem v-permission="'saimulti:config:update'" v-if="formArray.length > 0">
...
<script setup lang="ts">
  ...
  import api from '@/api/admin/system/config'
    ...
    core: {
      ...
      columnsFactory: () => [
        ...
        { prop: 'code', label: '配置标识', useHeaderSlot: true, width: 150 },
        {
          ...
          label: '租户配置',
          width: 100,
          formatter: (row: any) => (row.type === 1 ? '不可用' : '可用')
        }
      ]
    }
  })
