      ...
      <div class="flex-shrink-0 h-full max-md:w-full max-md:h-auto max-md:mb-5">
        <ElCard class="left-card art-card-xs flex flex-col h-full mt-0" shadow="never">
          ...
          <ElSpace wrap>
            ...
            <SaButton
              v-permission="'saimulti:dict:save'"
              type="primary"
              @click="typeShowDialog('add')"
            />
            <SaButton
              v-permission="'saimulti:dict:update'"
              type="secondary"
              @click="updateTypeDialog"
            />
            <SaButton
              v-permission="'saimulti:dict:destroy'"
              type="error"
              @click="deleteTypeDialog"
            />
          </ElSpace>
          <ArtTable
            rowKey="id"
...
<script setup lang="ts">
  ...
  import api from '@/api/admin/system/dict'
