        ...
        <ElCard class="tree-card art-card-xs flex flex-col h-full mt-0" shadow="never">
          <template #header>
            <div class="flex justify-between items-center">
              <b>附件分类</b>
              <SaButton
                v-permission="'saimulti:attachment:edit'"
                type="primary"
      ...
      <div class="flex flex-col flex-grow min-w-0">
        <ElCard class="art-table-card !mt-0" shadow="never">
          ...
          <div class="flex justify-between items-center mb-4">
            <ElSpace wrap>
              <ElUpload
                v-permission="'saimulti:system:uploadImage'"
                class="upload-btn"
          ...
          >
            <template #organization="{ row }">
              <span>{{ row.organ ? row.organ.organization_name : '中台' }}</span>
            </template>
            <!-- 操作列 -->
...
<script setup lang="ts">
  import api from '@/api/tool/attachment'
  import categoryApi from '@/api/tool/category'
  ...
  const {
    ...
    core: {
      ...
      columnsFactory: () => [
        { type: 'selection' },
        { prop: 'organization', label: '所属机构', width: 120, useSlot: true },
