  ...
  <div class="art-full-height">
    ...
    <ElCard class="art-table-card" shadow="never">
      ...
      <ArtTableHeader v-model:columns="columnChecks" :loading="loading" @refresh="refreshData">
        <template #left>
          <ElSpace wrap>
            <ElButton v-permission="'saimulti:crontab:edit'" @click="showDialog('add')" v-ripple>
              <template #icon>
      ...
      >
        ...
        <template #operation="{ row }">
          <div class="flex gap-2">
            <SaButton
              v-permission="'saimulti:crontab:run'"
              type="primary"
              icon="ri:play-fill"
              toolTip="运行任务"
              @click="handleRun(row)"
            ...
            <SaButton
              type="primary"
              v-permission="'saimulti:crontab:index'"
              icon="ri:history-line"
              toolTip="运行日志"
              @click="showTableDialog('edit', row)"
            />
            <SaButton
              v-permission="'saimulti:crontab:edit'"
              type="secondary"
              @click="showDialog('edit', row)"
            />
            <SaButton
              v-permission="'saimulti:crontab:destroy'"
              type="error"
